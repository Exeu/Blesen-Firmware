...
#define SECONDS_PER_TICK 16  // Bei RTC_WAKEUPCLOCK_CK_SPRE_16BITS ist ein Tick 16 Sekunden
#define DESIRED_MINUTES 10
...
static void Adv_Finished(void);

void Adv_Init() {
    HW_TS_Create(CFG_TIM_PROC_ID_ISR, &(AdvContext.Advertising_mgr_timer_Id), hw_ts_SingleShot, Adv_Finished);
}
...
static void Adv_Finished(void) {
    ...
    uint8_t wakeup_counter = (DESIRED_MINUTES * SECONDS_PER_MINUTE) / SECONDS_PER_TICK;
